...
Command commands[] = {
	...
	{ "env GIT_AUTHOR_NAME='' GIT_AUTHOR_EMAIL='' GIT_COMMITER_NAME='' GIT_COMMITER_EMAIL='' git commit --amend --reset-author", "commit using another author name", "reparar el nombre del autor"},
	...
	{ "gpg --keyserver pgp.mit.edu --send-keys [keyid]", "upload one key to keyserver", "push publick key"},
	...
	{ "ssh-keygen -f .ssh/known_hosts -R [host]", "remove a server from known hosts file", "key changed"},
	{ "ssh -N -L [ip-local]:[puerto-local]:[servidor]:[puerto-remoto] usuario@servidor", "forward a port using ssh", },
	{ "sudo chgrp -R [group] [dir]; sudo find [dir] -type d -exec chmod g+s {} ';'", "set default group for new files" },
	{ "sudo dpkg --remove --force-remove-reinstreq [pkg]", "remove broken packages", },
